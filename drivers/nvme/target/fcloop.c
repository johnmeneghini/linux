...
static ssize_t
fcloop_set_rport_marginal(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct fcloop_nport *nport;
	struct fcloop_ctrl_options opts;
...
static DEVICE_ATTR(set_rport_marginal, 0200, NULL, fcloop_set_rport_marginal);
...
static struct attribute *fcloop_dev_attrs[] = {
	...
	&dev_attr_set_rport_marginal.attr,
	NULL
};
