	...
	uint32_t start, uint8_t is_update)
{
	...
	if (!ha->optrom_buffer) {
		ql_log(ql_log_warn, vha, 0x7059,
		    "%s: Unable to allocate memory for optrom retrieval "
...
static int qla29xx_bsg_flash_block_read(struct bsg_job *bsg_job)
{
	...
	ql_log(ql_log_info, vha, 0x7062,
	       "%s: region 0x%x options Ox%x rw_length Ox%x offset Ox%x chunk_length Ox%x\n",
...
static int qla2900_bsg_dump_mpi(struct bsg_job *bsg_job)
{
	...
	ql_log(ql_log_info, vha, 0xffff,
	       "%s: mpi_address 0x%x mpi options Ox%x length Ox%x\n",
